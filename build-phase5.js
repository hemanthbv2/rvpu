...
const institutes = [
  { id: 'rvpu-north', altId: 'north', dir: 'rvpu-north-chatbot', name: 'RV PU College North', port: 3001, initial: 'N', color: '#E30613', glow: 'rgba(227,6,19,0.4)' },
  { id: 'rvpu-south', altId: 'south', dir: 'rvpu-south-chatbot', name: 'RV PU College South', port: 3002, initial: 'S', color: '#8B0000', glow: 'rgba(139,0,0,0.4)' },
  { id: 'rvpu-ecity', altId: 'ecity', dir: 'rvpu-ecity-chatbot', name: 'RV PU College Electronic City', port: 3003, initial: 'E', color: '#0B2545', glow: 'rgba(11,37,69,0.4)' },
  { id: 'rvpu-harohalli', altId: 'harohalli', dir: 'rvpu-harohalli-chatbot', name: 'RV PU College Harohalli', port: 3004, initial: 'H', color: '#C8102E', glow: 'rgba(200,16,46,0.4)' },
  { id: 'rvpu-mysore', altId: 'mysore', dir: 'rvpu-mysore-chatbot', name: 'RV PU College Mysuru', port: 3005, initial: 'M', color: '#E30613', glow: 'rgba(227,6,19,0.4)' },
  { id: 'ssmrvpu', altId: 'ssmrvpu', dir: 'ssmrvpu-chatbot', name: 'SSMRV PU College', port: 3006, initial: 'S', color: '#7A0000', glow: 'rgba(122,0,0,0.4)' },
  { id: 'nmkrvpu', altId: 'nmkrvpu', dir: 'nmkrvpu-chatbot', name: 'NMKRV PU College for Women', port: 3007, initial: 'W', color: '#EE9B54', glow: 'rgba(238,155,84,0.4)' }
];

function generateServerJs(inst) {
  return `/**
 * RVPU Central Command Center Server - ${inst.name}
...
const INST_NAME = ${JSON.stringify(inst.name)};
...
console.log('=== Updating server.js for all 7 RVPU local dashboards ===');

institutes.forEach(inst => {
  const serverPath = path.join(__dirname, inst.dir, 'dashboard', 'server.js');
  fs.writeFileSync(serverPath, generateServerJs(inst), 'utf8');
  console.log(`✓ Updated server.js for ${inst.name} on port ${inst.port}`);
});

console.log('=== All 7 Local Servers Successfully Updated! ===');
